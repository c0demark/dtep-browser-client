(function(angular, d3) {
	...
	function PaintingScreenController(
		...
		function createAndSetTemplatePaintingContainerDiv(id) {
			...
			// console.log($document[0].querySelector("#" + id));
		...
		function createBoxShadowFilterDef(svgContainerGtag) {
			...
			nodeBoxShadowFilter.append("feGaussianBlur").attrs({
				in: "SourceAlpha",
				stdDeviation: "3",
				result: "blur"
			});
			nodeBoxShadowFilter.append("feOffset").attrs({
				in: "blur",
				dx: "2",
				dy: "2",
				result: "offsetBlur"
		...
		function dragoverEventHandler(event) {
			var draggedLifecycleComponent = $scope.$parent.getDraggedLifecycleComponent();
			if (draggedLifecycleComponent) {
		...
		function dropEventHandler(event) {
			var draggedLifecycleComponent = $scope.$parent.getDraggedLifecycleComponent();
			var softwareListModalInstance = null;
			if (draggedLifecycleComponent) {
				softwareListModalInstance = $scope.$parent.openSoftwareListModal();
				softwareListModalInstance.opened
					.then(function(response) {
						$scope.$parent.fetchAllSoftwaresByComponentCode(
							draggedLifecycleComponent.componentCode
						);
					})
					.catch(function(errorResponse) {})
					.finally(function(notify) {});

				softwareListModalInstance.result
					.then(function(response) {
						// console.log(response);
						// console.log("software list model closed promise success is called");
						...
						var nodeBackgroundPatch = node
							.append("g")
							.classed(
								"dtep-curate-project-component-node-background-patch",
								true
							);
						...
						nodeBackgroundPatch
							...
							.append("rect")
							.classed(
								"dtep-curate-project-component-node-connection-point",
								true
							)
							.styles({
								width: "120px",
						...
						nodeBackgroundPatch
							...
							.append("line")
							.classed(
								"dtep-curate-project-component-node-connection-line",
								true
							)
							.attrs({
								x1: 0,
						...
						nodeBackgroundPatch
							.append("rect")
							.classed(
								"dtep-curate-project-component-node-box-with-shadow",
								true
							)
							.styles({
								width: "100px",
						...
						node
							...
							.append("image")
							.attrs({
								"xlink:href": draggedLifecycleComponent.iconURL
							})
							.styles({
								width: "40px",
								height: "40px",
						...
						softwareInfoGtag
							...
							.append("title")
							.text(function(data) {
								return draggedLifecycleComponent.componentName;
							});

						var softwareStatusGtag = node
							.append("g")
							.classed(
								"dtep-curate-project-component-node-software-status",
								true
							)
							.attrs({
						...
						node
							.on("mouseover", function(event) {
								...
								connectionPointContainer
									.select(
										"line.dtep-curate-project-component-node-connection-line"
									)
									.transition()
									.duration(300)
									.attrs({
						...
						//   $scope.$parent.unholdDraggedLifecycleComponent();
					})
					.catch(function(errorResponse) {})
					.finally(function(notify) {});
			}
